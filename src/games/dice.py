...
from src.config import SIZE, FPS
from src.utils import load_image, terminate, Button, Text, Text_Button, \
    load_sound
...
def start_game(screen):
    ...
    roll_sound = load_sound("roll-dice.wav")
    drop_sound = load_sound("drop-dice.wav")
    ...
    game = Game()
    exit_btn = Text_Button("Назад", (1060, 0), 20, screen)
    round_is_play = False
    running = True
    while running:
            ...
            if event.type == stop_rotate:
                ...
                if game.turn == 0:
                    ...
                    if point == 1:
                        ...
                        game.turn = 1
                    text_manager.add_text(f"{game.point_1 + point}", (0, 0, 0)
                                          , 70, 140, 50, "score1")
                    game.point_1 += point
                else:
                    if point == 1:
                    ...
                    text_manager.add_text(f"{game.point_2 + point}", (0, 0, 0)
                                          , 70, 830, 50, "score2")
                    game.point_2 += point
                drop_sound.play()
                dice.round_ended = True
                dice.point = point
                cont = Text_Button("Продолжить", (430, 300), 50, screen)
                save_btn = Text_Button("Сохранить очки", (430, 400), 40,
                                       screen)
        text_manager.update()
        ...
        if game.point_1 >= 100:
            round_is_play = False
            text_manager.add_text("Игрок 1 победил", (0, 0, 0), 50, 430, 300,
                                  "win")
            ...
            round_is_play = False
            text_manager.add_text("Игрок 2 победил", (0, 0, 0), 50, 430, 300,
                                  "win")
            play_btn = Button((490, 200), "play2.png", (170, 100), all_sprites)
            game.point_1 = 0
            game.point_2 = 0
